...
def c_sign(h_session, sign_flavor, data_to_sign, h_key, mech=None):
    ...
    :param mech: The mechanism to use, if None a blank mechanism will be created based on the sign_flavor (Default value = None)
    :returns: The result code, A python string represeting the signature
...
def c_verify(h_session, h_key, verify_flavor, data_to_verify, signature, mech=None):
    ...
    :param mech: The mechanism to use, if None is specified the mechanism will
        try to be automatically obtained (Default value = None)
    :returns: s The result code
