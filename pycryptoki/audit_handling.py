...
from pycryptoki.test_functions import LunaException, check_luna_exception, \
...
def ca_init_audit(slot, audit_pin, audit_label):
    if audit_pin == '':
        ...
        ret = CA_InitAudit(CK_SLOT_ID(slot), cast(audit_pin, CK_CHAR_PTR), CK_ULONG(len(audit_pin)), cast(audit_label, CK_CHAR_PTR))
    return ret
