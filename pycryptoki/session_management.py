...
def c_open_session(slot_num, flags=(CKF_SERIAL_SESSION | CKF_RW_SESSION)):
    ...
    @return: The result code, the session handle
    '''
    #OPEN SESSION
    ...
    ret = C_OpenSession(CK_SLOT_ID(slot_num), CK_FLAGS(flags), cast(arg3, CK_VOID_PTR), CK_NOTIFY(0), pointer(h_session))
...
def login(h_session, slot_num=1, password="userpin", user_type=1):
    ...
    @param slot_num: Slot index to login on
    ...
    logger.info("C_Login: Logging In. user_type=" + str(user_type) + ", slot=" + str(slot_num) + ", password=" + password)
...
def get_slot_info(description):
    ...
    @return: THe result code, a Python dictionary representing the slots
    '''
...
def c_get_token_info(slot_id):
    ...
    @return: The result code, A python dictionary representing the token info
    '''
    token_info = {}
...
def get_slot_dict():
    ...
    @return: A python dictionary of the available slots
    '''
    us_count = CK_ULONG(0)
...
def ca_create_container(h_session, storage_size, password='userpin', label='Inserted Token'):
    ...
    @param password: The password associated with the token
    @param label: The label associated with the token

    @return: The result code, The container number
    '''

    if password == '':
        container_number = CK_ULONG()
        logger.info("CA_CreateContainer: Inserting token with no SO storage_size=" + str(storage_size) + ", pin=" + password + ", label=" + label)
        ret = CA_CreateContainer(h_session, CK_ULONG(0), cast(create_string_buffer(label), CK_BYTE_PTR), CK_ULONG(len(label)), None,
                                 CK_ULONG(0), CK_ULONG(-1), CK_ULONG(-1), CK_ULONG(0), CK_ULONG(0), CK_ULONG(storage_size), byref(container_number))
        logger.info("CA_CreateContainer: Inserted token into slot " + str(container_number.value))
        return ret, container_number.value
    else:
        container_number = CK_ULONG()
        logger.info("CA_CreateContainer: Inserting token with no SO storage_size=" + str(storage_size) + ", pin=" + password + ", label=" + label)
        ret = CA_CreateContainer(h_session, CK_ULONG(0), cast(create_string_buffer(label), CK_BYTE_PTR), CK_ULONG(len(label)), cast(create_string_buffer(password), CK_BYTE_PTR),
                           CK_ULONG(len(password)), CK_ULONG(-1), CK_ULONG(-1), CK_ULONG(0), CK_ULONG(0), CK_ULONG(storage_size), byref(container_number))
        logger.info("CA_CreateContainer: Inserted token into slot " + str(container_number.value))
        return ret, container_number.value
...
def ca_delete_container_with_handle(h_session, container_handle):
    ...
    logger.info("CA_DeleteContainerWithHandle: Attempting to delete container with handle: {0}".format(container_handle))
