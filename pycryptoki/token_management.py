...
from pycryptoki.session_management import c_get_token_info, get_slot_info
...
def c_init_token(slot_num, password, token_label='Main Token'):
    ...
    @param token_label: The label to c_initialize the slot with
    ...
    if password == '':
        ...
        logger.info("C_InitToken: Initializing token. slot="+ str(slot_num) + ", label='" + token_label + "', password='" + password + "'")
        ret = C_InitToken(CK_ULONG(slot_num), cast(create_string_buffer(password), CK_CHAR_PTR), CK_ULONG(len(password)),  cast(create_string_buffer(token_label), CK_CHAR_PTR))
        return ret
...
def get_token_by_label(label):
    ...
    @return: The result code, The slot of the token
    '''
...
def c_get_mechanism_list(slot):
    ...
    @return: The result code, A python dictionary representing the mechanism list
    '''
    count = CK_ULONG()
...
def c_get_mechanism_info(slot, mechanism_type):
    ...
    @return: The result code, The mechanism info
    '''
