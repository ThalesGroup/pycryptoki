...
from defines import CKR_OK, CKM_DES_KEY_GEN, CKM_RSA_PKCS_KEY_PAIR_GEN
from pycryptoki.attributes import Attributes
from pycryptoki.cryptoki import CA_ClonePrivateKey, C_DeriveKey, \
    CK_SESSION_HANDLE
...
def c_destroy_object(h_session, h_object_value):
    ...
    @return: The resutl code from the C_DestroyObject operation
    '''
...
def _get_mechanism(flavor):
    ...
    @return: Returns a blank mechanism of type flavor
    '''
    mech = CK_MECHANISM()
...
def c_generate_key(h_session, flavor=CKM_DES_KEY_GEN, template=CKM_DES_KEY_GEN_TEMP): #TODO
    '''
...
def c_generate_key_pair(h_session, flavor=CKM_RSA_PKCS_KEY_PAIR_GEN, 
    ...
    @param h_session: Current session
    @param flavor: The flavor of the key to generate
    ...
    @return: Returns the result code, the public key's handle, and the private key's handle
    '''
    # INITALIZE VARIABLES
    if mech is None:
        mech = _get_mechanism(flavor)
...
def c_derive_key(h_session, h_base_key, template, mech_flavor, mech = None):
    ...
    @return: The result code, The derived key's handle
    '''
