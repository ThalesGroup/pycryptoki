...
Download gccxml from CVS (note: CVS seems to be blocked on the CVS network, you can get this from smb://172.20.11.83/ftp/forMike)
Compile gccxml
1) Recursively run dos2unix on gccxml source tree (find . -name *.* -exec dos2unix {} \;)
2) Create a folder in the same directory as gccxml is in called gccxml-build (ex /home/mhughes/gccxml and /home/mhughes/gccxml-build)
3) cd gccxml-build
...
def _parse_headers(xml_output, library_path=None):
    ...
    if (which("gccxml") == None and which("gccxml.exe") == None):
...
def _prepend_auto_file_warning(filename, comment):
    ...
    beginning_comment = beginning_comment + "'''\n\n"
