...
def assert_test_return_value(value, expected_value, message, print_on_success=True):
    ...
    @param print_on_success: Whether or not to print if the test case passes
    '''
...
def verify_object_attributes(h_session, h_object, expected_template):
    ...
    assert_test_case(attr == expected_attributes, "Object " + str(h_object.value) + " attributes match expected attributes")
...
def verify_object_exists(h_session, h_object, should_exist=True):
    ...
    @param should_exist: Whether or not the parameter should exist (Use to determine how this function asserts)
    '''
    # VERIFY OBJECT EXISTS
    h_object = CK_OBJECT_HANDLE(h_object)
    us_size = CK_ULONG()
    ...
    if should_exist:
        ...
        assert_test_return_value(ret, CKR_OBJECT_HANDLE_INVALID, "Getting object " + str(h_object.value) + "'s size", True)
        assert_test_case(us_size.value <= 0, "Object " + str(h_object.value) + " size is greater than zero.", False)
...
def make_error_handle_function(luna_function):
    ...
    def luna_function_exception_handle(*args, **kwargs):
        ...
        if isinstance(return_tuple, tuple):
            ...
            raise Exception("Functions wrapped by the exception handler should return a tuple or just the long representing Luna's return code.")
