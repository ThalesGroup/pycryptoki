...
class TestHSMManagementFunctions(object):
    ...
    def test_performselftest(self, test_type):
        ...
        ret, data = c_performselftest(self.admin_slot, test_type, input_data, input_length)
        assert ret == CKR_OK, (
            "Return code should be "
            + ret_vals_dictionary[CKR_OK]
            + " not "
    ...
    def test_settokencertsignature(self):
        ...
        signature = list(range(4000))
        signature_length = 4000

        ret = ca_settokencertificatesignature(
            self.h_session, access_level, customer_id, pub_template, signature, signature_length
        )
    ...
    @pytest.mark.xfail(hsm_config["user"] == "CO", reason="Unable to run on CO slot")
    def test_hainit(self):
        """Tests performs HA init"""
        ret, pubkey_h, prikey_h = c_generate_key_pair(
            self.h_session,
            CKM_RSA_PKCS_KEY_PAIR_GEN,
            CKM_RSA_PKCS_KEY_PAIR_GEN_PUBTEMP,
            CKM_RSA_PKCS_KEY_PAIR_GEN_PRIVTEMP,
        ...
        assert pubkey_h > 0, "The public key handle returned should be non zero"
        assert prikey_h > 0, "The private key handle returned should be non zero"
    ...
    @pytest.mark.skipif(
        condition=not hsm_config["is_ped"] or hsm_config["user"] == "CO",
